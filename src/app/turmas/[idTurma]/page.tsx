...
function getDocumentosByTurmaId(turmaId: number): Documento[] { 
  return documentos.filter((d) => d.turmaId === turmaId)
}
...
function DocumentoCard({
  ...
  return (
    <div className={styles.cardDocumento}>
      <div className={`${styles.cabecalhoDocumento} ${isPdf ? styles.cabecalhoPdf : styles.cabecalhoDocx}`}>
        {isPdf ? (
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M6 2h9l5 5v13a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2z" fill="rgba(255,255,255,0.15)" stroke="#fff" strokeWidth="1.4" />
            <path d="M15 2v5h5" stroke="#fff" strokeWidth="1.4" />
          ...
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M6 2h9l5 5v13a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2z" fill="rgba(255,255,255,0.15)" stroke="#fff" strokeWidth="1.4" />
            <path d="M15 2v5h5" stroke="#fff" strokeWidth="1.4" />
            <text x="12" y="17" textAnchor="middle" fontSize="7" fontWeight="700" fill="#fff">W</text>
          </svg>
        )}
      </div>

      <div className={styles.corpoDocumento}>
        <p className={styles.nomeDocumento}>{documento.nome}</p>

        <div className={styles.acoesDocumento}>
          <button type="button" className={styles.botaoAbrir} onClick={() => onAbrir(documento)}>
            Abrir
          </button>
          <button type="button" className={styles.botaoExcluirDocumento} onClick={() => onExcluir(documento)}>
            Excluir
          </button>
        </div>
      </div>
...
//Isso é o responsável por fazer o [idTurma] funcionar
...
export default function DetalhesDaTurma({ params }: DetalhesDaTurmaProps) {
  const router = useRouter() //Responsável por permitir o uso de .back ou .push futuramente

  const idTurma = Number(params.idTurma) //Transforma o ID que era '1' (string) em 1 (number)
  ...
  function handleAdicionarDocumento() {
    console.log('Adicionar documento para a turma:', turma!.id)
  }
      ...
      >
          <section className={styles.banner}>
            <div
              ...
              style={{ backgroundImage: `url(${turma.imagem})` }}
            >
              <div className={styles.bannerOverlay} />
            </div>

            <div className={styles.bannerConteudo}>
              <button type="button" className={styles.botaoVoltar} onClick={handleVoltar} aria-label="Voltar">
              ...
              <div className={styles.bannerTextos}>
                ...
                <h1 className={styles.nomeTurmaGrande}>{nomeTurma}</h1>
              </div>
            </div>
          </section>
          ...
          <section className={styles.conteudoInferior}>
            <div className={styles.documentosArea}>
              <div className={styles.documentosCabecalho}>
                <h2 className={styles.documentosTitulo}>Documentos da Disciplina</h2>
                <button type="button" className={styles.botaoAdicionar} onClick={handleAdicionarDocumento}>
                  ADICIONAR DOCUMENTO +
                </button>
              </div>
            ...
            <aside className={styles.lateralDireita}>
              <div className={styles.mediaCard}>
                <p className={styles.mediaTitulo}>Média da turma:</p>
                <p className={styles.mediaValor}>{turma.media}</p>
              </div>

              <div className={styles.acoesLista}>
                <button type="button" className={styles.botaoAcao} onClick={handleFazerChamada}>
                  <svg className={styles.iconeAcao} viewBox="0 0 24 24" fill="none" aria-hidden="true">
                ...
                <button type="button" className={styles.botaoAcao} onClick={handleLancarNotas}>
                  <svg className={styles.iconeAcao} viewBox="0 0 24 24" fill="none" aria-hidden="true">
                ...
                <button type="button" className={styles.botaoAcao} onClick={handleMandarNotificacao}>
                  <svg className={styles.iconeAcao} viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path
